...
public class qLearning : MonoBehaviour {
    ...
    void Learning() {
        /*
         read from file into qTable
         perform the same opreastion
         remove the epsilon so no randoness only max
         */


        //while (episode != 5) {
        //    Debug.Log("Episode : " + episode);
        //    sIndex = Random.Range(0, 40); // sin
        //    aIndex = Random.Range(0, 44); // ain
        ...
        qTable[aIndex, sIndex] = rMatrix[aIndex, sIndex] + (gamma * MaxAction(sIndex)); //assinging
        //q(0,2) = 300; q(0,2) = ±±§§§§§§§~
    ...
    void PrintToFile() {
        ...
        for (int rows = 0; rows < finalRow; rows++) {
            for (int coloumn = 0; coloumn < finalColumn; coloumn++) {
                writer.Write(qTable[rows,coloumn] + " ");
            }
